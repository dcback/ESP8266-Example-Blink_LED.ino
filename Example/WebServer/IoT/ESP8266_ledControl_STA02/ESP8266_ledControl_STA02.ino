...
void handleForm1()
{
   String s1 = server.arg("button1"); // Get button state
   if (s1 == "ON")
   {
       digitalWrite(LED1, LOW); // Turn LED1 on(LED is reverse connect)
       LED_state1 = "ON";       // Change status to ON
   }
   if (s1 == "OFF")
   {
       digitalWrite(LED1, HIGH); // Turn LED1 off(LED is reverse connect)
...
void handleForm2()
{
   String s2 = server.arg("button2"); //Get button state
   if (s2 == "ON")
   {
       digitalWrite(LED2, LOW); // Turn LED2 on(LED is reverse connect)
       LED_state2 = "ON";       // Change status to ON
   }
   if (s2 == "OFF")
   {
       digitalWrite(LED2, HIGH); // Turn LED2 off(LED is reverse connect)
...
void setup()
{
   ...
   server.on("/form1", handleForm1); // Request sent when click on button
   server.on("/form2", handleForm2); // Request sent when click on button
