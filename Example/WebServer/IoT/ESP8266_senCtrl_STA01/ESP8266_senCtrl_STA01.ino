...
void handleForm()
{
    ...
    {
        L2Status = "ON";
        digitalWrite(Load2, LOW);    // Load2 Turned on
    }
    if (L_state == "OFF2")
    {
        L2Status = "OFF";
        digitalWrite(Load2, HIGH);   // Load2 Turned off
    }

    if (L_state == "ON3")
    {
        L3Status = "ON";
        digitalWrite(Load3, LOW);    // Load3 Turned on
    }
    if (L_state == "OFF3")
    {
        L3Status = "OFF";
        digitalWrite(Load3, HIGH);   // Load3 Turned off
    }

    if (L_state == "ON4")
    {
        L4Status = "ON";
        digitalWrite(Load4, LOW);    // Load4 Turned on
    }
    if (L_state == "OFF4")
    {
        L4Status = "OFF";
        digitalWrite(Load4, HIGH);   // Load4 Turned off
    }

    server.sendHeader("Location", "/"); //Send web browser to root location
    server.send(302, "text/html", "");  //Send browser 302 redirect go back
}
//=================================================================
//                    Power on setup
//===================================================================
